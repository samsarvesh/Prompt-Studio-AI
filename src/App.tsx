...
export default function App() {
  ...
  // Check for shared prompt on mount
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const shareId = params.get('share');
    if (shareId) {
      loadSharedPrompt(shareId);
    }
  }, []);

  const loadSharedPrompt = async (id: string) => {
  ...
  const generatePrompt = async () => {
    ...
    if (!ai || !key) {
      console.error("AI initialization failed. Key found:", !!key);
...
Ensure the improvedPrompt is formatted with clear headings and bullet points for readability.`,
      ...
      console.error("Generation failed. Full error object:", error);
      const errorMessage = error?.message || "Unknown error";
      const isLeakedKey = errorMessage.toLowerCase().includes("leaked");
      const isQuotaError = errorMessage.toLowerCase().includes("quota") || errorMessage.includes("429");
      const isLocationError = errorMessage.toLowerCase().includes("location") || errorMessage.toLowerCase().includes("region") || errorMessage.toLowerCase().includes("supported");
      const isModelError = errorMessage.toLowerCase().includes("model") || errorMessage.includes("404");
      const isAuthError = errorMessage.includes("API_KEY_INVALID") || errorMessage.includes("403") || errorMessage.includes("401");
      const isSafetyError = errorMessage.includes("SAFETY") || errorMessage.includes("blocked");
      
      let displayMessage = `Failed to generate prompt: ${errorMessage}.`;
      let displayExplanation = "The AI service encountered an error.";

      if (isLeakedKey) {
        displayMessage = "Your API key has been flagged as leaked and disabled for security.";
        displayExplanation = "Google automatically disables API keys that are found in public repositories or exposed online. Please go to the Settings menu and update your GEMINI_API_KEY with a new one from aistudio.google.com.";
      } else if (isQuotaError) {
        displayMessage = "API Quota Exceeded.";
        displayExplanation = "You have reached the free tier limit for the Gemini API. Please wait a moment or check your usage at aistudio.google.com.";
      } else if (isLocationError) {
        displayMessage = "Service not available in your region.";
        displayExplanation = "The Gemini API is not currently available in your geographic location. You may need to use a VPN or wait for regional support.";
      } else if (isModelError) {
        displayMessage = "Model not found or unavailable.";
        displayExplanation = "The requested AI model (gemini-3-flash-preview) might be undergoing maintenance or is not available for your API key.";
      } else if (isAuthError) {
        displayMessage = "Invalid or missing API Key. Please ensure GEMINI_API_KEY is correctly set in your environment variables.";
        displayExplanation = "Authentication failed. The application cannot communicate with the AI service without a valid key.";
      } else if (isSafetyError) {
        displayMessage = "The prompt was flagged by safety filters.";
        displayExplanation = "Your input or the generated output triggered the AI's safety guidelines. Try rephrasing your request.";
      } else {
        displayExplanation = "This could be due to a temporary service outage or network issue. Check your browser console (F12) for technical details.";
      }

      setResult({
        title: "Architecture Error",
        improvedPrompt: displayMessage,
      ...
      <AnimatePresence>
        {isLoadingShared && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-white/80 dark:bg-[#0A0B0E]/80 backdrop-blur-md z-[60] flex flex-col items-center justify-center gap-4"
          >
            <div className="relative">
              <div className="w-16 h-16 border-4 border-brand-200 dark:border-brand-900 rounded-full animate-pulse" />
              <Loader2 className="w-8 h-8 text-brand-600 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 animate-spin" />
            </div>
            <div className="text-center space-y-1">
              <h3 className="font-display font-bold text-lg text-gray-900 dark:text-white">Retrieving Architect's Vision</h3>
              <p className="text-sm text-gray-500 dark:text-gray-400">Fetching shared prompt data...</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={sharePrompt}
                      disabled={isSharing}
                      className="flex items-center gap-2 px-3 py-1.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-[10px] font-bold hover:bg-gray-50 dark:hover:bg-gray-700 transition-all shadow-sm disabled:opacity-50"
                    >
                      {isSharing ? <Loader2 className="w-3 h-3 animate-spin" /> : <Share2 className="w-3 h-3" />}
                      <span>{shareUrl && copied ? 'Link Copied!' : 'Share Prompt'}</span>
                    </button>
                    <button
                      onClick={() => copyToClipboard(result.improvedPrompt)}
                      className="flex items-center gap-2 px-3 py-1.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-[10px] font-bold hover:bg-gray-50 dark:hover:bg-gray-700 transition-all shadow-sm"
                    >
                      {copied && !shareUrl ? <Check className="w-3 h-3 text-green-500" /> : <Copy className="w-3 h-3" />}
                      <span>{copied && !shareUrl ? 'Copied!' : 'Copy Prompt'}</span>
                    </button>
                  </div>
                </div>
                ...
                <div className="relative z-10 space-y-4">
                  ...
                  <ul className="space-y-2 text-xs text-brand-100 font-light">
                    <li className="flex gap-2">
                      ...
                      <span>Define your <span className="font-bold text-white">audience</span> to set complexity.</span>
                    </li>
                    <li className="flex gap-2">
                      <div className="w-1 h-1 bg-brand-300 rounded-full mt-1.5 shrink-0" />
                      <span>Use <span className="font-bold text-white">Negative Constraints</span> to avoid pitfalls.</span>
                    </li>
                  </ul>
                </div>
              </section>
